...
\begin{figure}[H]
    ...
    \end{center}
    \caption{Global overview of water scarcity and agricultural water demand under climate change scenarios. \textcolor{blue}{Data sources: Water Scarcity Index (WSI) and projected changes derived from \citet{ipcc2022ar6}; agricultural water withdrawal statistics from \citet{fao2020state}. Software used: QGIS v.~3.44 for mapping; R v.~4.6.1 and ggplot2 v.~4.0.3 for bar charts and pie charts; Inkscape v.~1.4.4 for dashboard assembly. All map data and projections are author's original compilation from the cited open-access sources. Dashboard} source: author.}
    \label{fig01}
\end{figure}
...
2000s--2015 &
ML Models &
Support Vector Machines (SVM), Random Forest (RF), Decision Trees, \textcolor{blue}{$k$-Nearest Neighbors (kNN)}, Artificial Neural Networks (ANN) &
...
\citep{Barik2020, flint2018increasing, Hudson1994, Rawls2003} \\ \hline

\textcolor{blue}{Tortuosity} &
\textcolor{blue}{Ratio of the actual path length of solute movement through soil pores to the straight-line distance; describes the sinuosity of the pore network} &
\textcolor{blue}{Controls effective diffusion and dispersion of solutes, nutrients, and contaminants in the vadose zone; critical for predicting leaching risk} &
\textcolor{blue}{Pore geometry, soil structure, water content, clay content} &
\textcolor{blue}{\citep{simunek2008hydrus, vereecken2014soil}} \\ \hline

\textcolor{blue}{Pore-Water Velocity} &
\textcolor{blue}{Average linear velocity of water (and dissolved solutes) through the soil pore space; equals Darcy flux divided by volumetric water content} &
\textcolor{blue}{Governs the rate of solute transport, nutrient leaching, and pesticide movement toward groundwater} &
\textcolor{blue}{Hydraulic conductivity, matric potential gradient, water content, pore-size distribution} &
\textcolor{blue}{\citep{simunek2008hydrus, Dane2002}} \\ \hline

\textcolor{blue}{Dispersion Coefficient} &
\textcolor{blue}{Parameter quantifying the spreading of a solute plume due to mechanical dispersion and molecular diffusion during transport through the soil} &
\textcolor{blue}{Determines the spatial extent of contaminant plumes, nutrient front migration, and mixing in the vadose zone; essential for solute transport modelling} &
\textcolor{blue}{Pore-water velocity, dispersivity, diffusion coefficient, soil texture and structure} &
\textcolor{blue}{\citep{simunek2008hydrus, vereecken2014soil}} \\

\end{longtable}
\normalsize
%============= TABLE ==================================<

Soil texture---the relative proportions of sand, silt, and clay fractions---exerts a primary control on hydraulic behaviour, Figure \ref{fig02}. Sandy soils are characterised by large inter-particle pores, rapid drainage, low water-holding capacity, and high saturated hydraulic conductivity ($K_s$). Clay-rich soils, by contrast, possess predominantly micropores and mesopores that confer high water retention and low $K_s$, but are also prone to shrinkage-induced cracking upon desiccation and to surface sealing under rainfall impact. Loamy soils, occupying an intermediate textural position, generally offer the most favourable combination of water retention and aeration for crop production. The United States Department of Agriculture (USDA) and the Food and Agriculture Organization (FAO) textural classification schemes provide internationally standardised frameworks for communicating soil texture, and texture-based pedotransfer functions (PTFs) are routinely \textcolor{blue}{employed to} estimate hydraulic parameters from routinely measured soil properties \citep{wosten2001pedotransfer, Lemenkova202302, Lemenkova202314}.

\begin{figure}[H]
    \begin{center}
	\hspace{0pt}\includegraphics[width=14.0cm]{Figure_02.png}
    \end{center}
    \caption{Conceptual diagram scheme of soil--water interaction processes in agricultural systems. \textcolor{blue}{Abbreviations: LAI = Leaf Area Index.} \textcolor{blue}{Software: Inkscape v. 1.4.4 for vector elements, GIMP, v. 3.2.4 for raster elements; OpenClipart (openclipart.org) for free SVG clipart of plants, clouds, sun. Infographic} source: author.}
    \label{fig02}
\end{figure}
...
\noindent where $S_e = (\theta - \theta_r)/(\theta_s - \theta_r)$ is the effective saturation and $l$ is a pore-connectivity parameter commonly set to 0.5. \textcolor{blue}{This formulation is consistent with Eq.~13 in \citet{simunek2008hydrus}.} Estimating these parameters for field conditions is a significant practical challenge that AI-based PTFs increasingly seek to address.
...
\begin{figure}[H]
    ...
    \end{center}
    \caption{Comparison between physics-based and AI-based soil-water modelling frameworks. \textcolor{blue}{Abbreviations in panel equations: $\partial\theta/\partial t$ = rate of change of volumetric water content; $K$ = unsaturated hydraulic conductivity; $h$ = matric potential head; $z$ = vertical coordinate; $S$ = root water uptake sink term; $C$ = solute concentration; $D$ = hydrodynamic dispersion coefficient; $q$ = pore-water flux density (Darcy velocity); $\lambda$ = first-order degradation rate; $S_C$ = solute source/sink term; $P$, $I$, $ET$, $R$, $D$, $\Delta S$ = precipitation, irrigation, evapotranspiration, runoff, drainage, and storage change in the water balance.} \textcolor{blue}{Software used: \LaTeX \space for equations and TexText plugin for rendering \LaTeX \space equations; Inkscape v. 1.4.4 for vector elements; TikZ package for neural network diagram (panel B); Material Design Icons for symbols. Flowchart} source: author.}
    \label{fig03}
\end{figure}
...
where $\theta_k$ denotes the local model parameters for client $k$, \textcolor{blue}{$t \in \{0, 1, \dots, T-1\}$ is the local iteration index with $T$ denoting the total number of local gradient steps per communication round,} $\eta$ is the learning rate, and $\mathcal{L}_k$ is the local loss function computed on the client dataset.
...
Deep learning approaches \textcolor{blue}{are increasingly applied to estimate} hydraulic conductivity, infiltration rates, and water retention curves from soil texture and environmental datasets.
...
In irrigation scheduling applications, SHAP-based attribution \textcolor{blue}{enables identification of} which meteorological variables---evapotranspiration demand, antecedent rainfall, vapour pressure deficit---most strongly drive irrigation timing recommendations, enabling agronomists to audit model decisions against domain knowledge \citep{zhou2023xai}.
...
Physics-informed neural networks (PINNs) offer opportunities to integrate physical laws with data-driven learning for improved generalization. The integration of domain physical knowledge with deep learning through \textcolor{blue}{physics-guided loss functions} represents one of the most promising frontiers for improving the reliability and generalisability of AI-based soil--water models \citep{karniadakis2021physics}. 
...
\citep{pylianidis2021digital, verdouw2021digital}. An agricultural digital twin integrates data streams from field sensors, satellite observations, weather forecasts, and agronomic management records into a dynamically updated state representation of the soil--water--plant--atmosphere system, which serves as both a predictive engine for forward simulation under different irrigation and climate scenarios and a decision-support interface for farm managers and water resource planners. 
