...
\subsubsection{Pedotransfer Functions and Estimation of Soil Hydraulic Parameters}

The estimation of hydraulic parameters (saturated hydraulic conductivity $K_s$, water retention curve parameters $\alpha$, $n$, $\theta_r$, $\theta_s$) from more easily measured soil characteristics such as texture, bulk density, and organic matter is the domain of pedotransfer functions (PTFs). Traditional PTFs are regression-based equations fitted to relatively small, geographically limited soil datasets.

A general linear pedotransfer function may be expressed as:
...
where $Y$ denotes the predicted hydraulic parameter, $X_i$ are soil predictor variables (e.g., clay content, bulk density, organic carbon), $\beta_i$ are regression coefficients, and $\varepsilon$ is the residual error.

The van Genuchten soil water retention model, commonly parameterised using PTFs, is given by:
...
where $\theta(h)$ is the volumetric water content at matric potential $h$, $\theta_r$ and $\theta_s$ are the residual and saturated water contents, respectively, $\alpha$ and $n$ are empirical shape parameters, and

\begin{equation}
m = 1 - \frac{1}{n}.
\end{equation}

Saturated hydraulic conductivity is often modelled empirically as:

\begin{equation}
K_s
=
f(\text{sand},\text{clay},\rho_b,\text{OM}),
\end{equation}

where $\rho_b$ is bulk density and OM denotes organic matter content.

Deep learning PTFs trained on large, globally compiled databases (e.g., the UNSODA, HYDRUSbase, and WRB datasets) have substantially outperformed traditional regression PTFs in independent validation studies, particularly for soils with extreme textural compositions or unusual structural properties.\citep{zhang2018deep}.
